...
def tinify(command: CommandRequest):
    ...
    optional_text = f'Which 1 of these files do you think would be most important to solve this issue: {json.dumps(github_files)} Return as a json with "file_name" as the key and should have only 1 value. Do not give as a code snippet.'
    required_files = send_openAI_request(prompt, optional_text)
    messages.append({"role": "user", "content": f'{prompt}. {optional_text}'})
    messages.append({"role": "assistant", "content": f'{required_files}'})
    ...
    optional_text = f'Modify this file (only if it solves the problem) to solve the problem. Only return the new code snippet (if nothing should be changed, return the original file content): {file_content}'
    gpt_suggestion = send_openAI_request(prompt, optional_text)
    messages.append({"role": "user", "content": f'{prompt}. {optional_text}'})
    messages.append({"role": "assistant", "content": f'{gpt_suggestion}'})
    ...
    messages.append({"role": "user", "content": "Are you sure about this or do you want to correct your answer? (answer with a 1 or 0 and not include a full stop.)"})
    messages.append({"role": "assistant", "content": f'{want_to_reflect}'})

    if int(want_to_reflect):
        gpt_suggestion = send_openAI_request(prompt, optional_text=f'Correct Yourself.')
